...
exports.registerDefaultExecutors = registerDefaultExecutors;
...
class SlackExecutor {
    ...
    formatSlackMessage(context, config) {
        ...
        if ('verdict' in result) {
            return this.formatSingleRegressionMessage(result, context.timestamp);
        }
        else {
            return this.formatBatchResultMessage(result, context.timestamp);
        }
    }
    formatSingleRegressionMessage(result, timestamp) {
    ...
    formatBatchResultMessage(result, timestamp) {
...
class PagerDutyExecutor {
    ...
    createPagerDutyEvent(context, config) {
        const result = context.result;
        if ('verdict' in result) {
...
exports.PagerDutyExecutor = PagerDutyExecutor;
/**
...
class EmailExecutor {
    ...
    formatEmailContent(context, template) {
        if (template) {
 ...
 */
function registerDefaultExecutors(engine) {
