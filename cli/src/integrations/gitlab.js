...
exports.formatRegressionForMR = formatRegressionForMR;
exports.formatBatchResultsForMR = formatBatchResultsForMR;
exports.postGitLabMRNote = postGitLabMRNote;
exports.upsertGitLabMRNote = upsertGitLabMRNote;
exports.createGitLabCommitDiscussion = createGitLabCommitDiscussion;
exports.updateGitLabMRStatus = updateGitLabMRStatus;
exports.getGitLabConfigFromEnv = getGitLabConfigFromEnv;
const axios_1 = __importDefault(require("axios"));
/**
 * Format regression result as markdown for MR note
 */
function formatRegressionForMR(result, options = {}) {
 ...
 */
function formatBatchResultsForMR(batchResult, options = {}) {
    const { includeDetails = true } = options;
 ...
 */
async function postGitLabMRNote(config, note) {
    ...
    try {
        await axios_1.default.post(url, { body: note }, {
            headers: {
 ...
 */
async function upsertGitLabMRNote(config, note, marker = '<!-- vitals-regression-note -->') {
    const baseUrl = config.baseUrl || 'https://gitlab.com';
    const projectId = encodeURIComponent(config.projectId);
    const notesUrl = `${baseUrl}/api/v4/projects/${projectId}/merge_requests/${config.mrIid}/notes`;
    try {
        // Fetch existing notes
        const { data: notes } = await axios_1.default.get(notesUrl, {
            headers: {
                'PRIVATE-TOKEN': config.token
            }
        });
        const markedNote = `${marker}\n${note}`;
        const existingNote = notes.find((n) => n.body?.includes(marker));
        if (existingNote) {
            // Update existing note
            await axios_1.default.put(`${notesUrl}/${existingNote.id}`, { body: markedNote }, {
                headers: {
                    'PRIVATE-TOKEN': config.token,
                    'Content-Type': 'application/json'
                }
            ...
            // Create new note
            await axios_1.default.post(notesUrl, { body: markedNote }, {
                headers: {
                    'PRIVATE-TOKEN': config.token,
                    'Content-Type': 'application/json'
                }
 ...
 */
async function createGitLabCommitDiscussion(config, result, message) {
    const baseUrl = config.baseUrl || 'https://gitlab.com';
    const projectId = encodeURIComponent(config.projectId);
    const url = `${baseUrl}/api/v4/projects/${projectId}/repository/commits/${config.commitSha}/discussions`;
    try {
        await axios_1.default.post(url, { body: message }, {
            headers: {
                'PRIVATE-TOKEN': config.token,
                'Content-Type': 'application/json'
            }
 ...
 */
async function updateGitLabMRStatus(config, status, description) {
    const baseUrl = config.baseUrl || 'https://gitlab.com';
    const projectId = encodeURIComponent(config.projectId);
 ...
 */
function getGitLabConfigFromEnv() {
