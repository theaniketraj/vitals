...
exports.bootstrapConfidenceInterval = bootstrapConfidenceInterval;
exports.permutationTest = permutationTest;
exports.selectStatisticalTest = selectStatisticalTest;
const welch_1 = require("./welch");
/**
...
function kolmogorovSmirnovTest(sample1, sample2) {
    ...
    for (const point of allPoints) {
        ...
        while (i < n1 && sorted1[i] <= point)
            i++;
        while (j < n2 && sorted2[j] <= point)
            j++;
        const cdf1 = i / n1;
        const cdf2 = j / n2;
 ...
 */
function bootstrapConfidenceInterval(sample1, sample2, iterations = 1000, confidenceLevel = 0.95) {
    const diffs = [];
    for (let i = 0; i < iterations; i++) {
        // Resample with replacement
        const resample1 = resampleWithReplacement(sample1);
        const resample2 = resampleWithReplacement(sample2);
        const mean1 = (0, welch_1.mean)(resample1);
        const mean2 = (0, welch_1.mean)(resample2);
 ...
 */
function permutationTest(sample1, sample2, iterations = 1000) {
    const mean1 = (0, welch_1.mean)(sample1);
    const mean2 = (0, welch_1.mean)(sample2);
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        const permMean1 = (0, welch_1.mean)(perm1);
        const permMean2 = (0, welch_1.mean)(perm2);
 ...
 */
function resampleWithReplacement(sample) {
    const resampled = [];
 ...
 */
function selectStatisticalTest(sample1, sample2) {
    const n1 = sample1.length;
    const n2 = sample2.length;
 ...
 */
function checkNormality(sample) {
    const m = (0, welch_1.mean)(sample);
    const v = (0, welch_1.variance)(sample, m);
