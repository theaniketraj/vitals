...
exports.CrossSourceCorrelator = void 0;
exports.regressionToEvent = regressionToEvent;
exports.incidentToEvent = incidentToEvent;
/**
 * Cross-Source Correlator
 */
class CrossSourceCorrelator {
    constructor(regressionDb, knowledgeGraph, dependencyMapper) {
     ...
     */
    async correlateIncident(incidentId, timeWindow) {
        // Get incident from knowledge graph
        const incidentNode = await this.knowledgeGraph.getNode(`incident-${incidentId}`);
        if (!incidentNode) {
        ...
        const candidates = this.eventsByTime.filter(event => event.timestamp >= timeWindow.start &&
            event.timestamp <= timeWindow.end);
        // Correlate events
        const correlatedEvents = [];
        const affectedServices = new Set(incidentProps.affected_services);
     ...
     */
    async correlateRegression(regressionId) {
        ...
        const candidates = this.eventsByTime.filter(event => event.id !== regressionId &&
            event.timestamp >= timeWindow.start &&
            event.timestamp <= timeWindow.end);
        const correlatedEvents = [];
        const affectedServices = new Set([primaryEvent.service]);
        for (const event of candidates) {
            const correlation = this.calculateCorrelation(primaryEvent, event);
            if (correlation.score > 0.5) {
                correlatedEvents.push({
    ...
    async detectAnomalies(query) {
        ...
        const events = this.eventsByTime.filter(event => event.timestamp >= query.start_time &&
        ...
        for (let i = 1; i < events.length; i++) {
            ...
            if (prev.service !== curr.service || prev.source !== curr.source)
                continue;
     ...
     */
    async queryCorrelations(query) {
        let results = this.eventsByTime.filter(event => event.timestamp >= query.start_time &&
            event.timestamp <= query.end_time);
        // Apply filters
    ...
    async autoCorrelate(newEvent) {
        // Find events in time window (±15 minutes)
        const windowMs = 15 * 60 * 1000;
        const candidates = this.eventsByTime.filter(event => event.id !== newEvent.id &&
    ...
    calculateCorrelation(event1, event2) {
        ...
        if (deps) {
            const hasDependency = Array.from(deps.values()).some(dep => (dep.from_service === event1.service && dep.to_service === event2.service) ||
    ...
    determineLikelyRootCause(correlatedEvents) {
    ...
    generateRecommendations(primaryEvent, correlatedEvents) {
...
exports.CrossSourceCorrelator = CrossSourceCorrelator;
/**
 * Helper to convert regression to correlation event
 */
function regressionToEvent(regression) {
    return {
        id: regression.id,
        timestamp: regression.timestamp,
        source: 'regressions',
 ...
 */
function incidentToEvent(incident) {
