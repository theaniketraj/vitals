...
exports.ServiceDependencyMapper = void 0;
exports.exportToDot = exportToDot;
...
class ServiceDependencyMapper {
    constructor(basePath = '~/.vitals/service-dependencies') {
        this.loaded = false;
     ...
     */
    async extractFromTraces(traces) {
        await this.ensureLoaded();
     ...
     */
    async findCriticalPath(fromServiceId, toServiceId) {
        await this.ensureLoaded();
        ...
        const visited = new Set([fromServiceId]);
     ...
     */
    async analyzeServiceImpact(serviceId) {
        await this.ensureLoaded();
        ...
        const criticalityScore = this.calculateCriticalityScore(downstreamAll.size, upstreamAll.size, serviceId);
        return {
     ...
     */
    async getServiceHealthSummary(serviceId) {
        await this.ensureLoaded();
    ...
    async findAllDownstream(serviceId) {
    ...
    async findAllUpstream(serviceId) {
    ...
    calculateCriticalityScore(downstreamCount, upstreamCount, serviceId) {
        let score = 0;
    ...
    async load() {
        ...
        try {
            ...
            const dependencies = JSON.parse(depsData);
...
exports.ServiceDependencyMapper = ServiceDependencyMapper;
/**
 * Helper to visualize service map as DOT format (Graphviz)
 */
function exportToDot(serviceMap) {
    const lines = [];
