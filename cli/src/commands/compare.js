...
function registerCompareCommand(program) {
    program
        ...
        try {
            ...
            const baselineData = await (0, prometheus_1.fetchRangeMetrics)({
            ...
            const comparisonData = await (0, prometheus_1.fetchRangeMetrics)({
                url: options.prometheusUrl
            }, {
                metric: options.metric,
                label: options.service,
            ...
            const baselineMean = baselineData.reduce((sum, v) => sum + v, 0) / baselineData.length;
            const comparisonMean = comparisonData.reduce((sum, v) => sum + v, 0) / comparisonData.length;
